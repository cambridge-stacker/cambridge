local RecordsScene = Scene:extend()

RecordsScene.title = "Records"
RecordsScene.restart_message = false

local menu_screens = {
	PlayerCardScene,
	HighscoresScene,
	ReplaySelectScene,
}


function RecordsScene:new()
	self.menu_state = 1
	self.safety_frames = 2
	DiscordRPC:update({
		details = "In menus",
		state = "Viewing records",
		largeImageKey = "settings",
	})
end

function RecordsScene:update()
	self.safety_frames = self.safety_frames - 1
	if self.das_up or self.das_down then
		self.das = self.das + 1
	else
		self.das = 0
	end
	if self.das >= config.menu_das then
		local change = 0
		if self.das_up then
			change = -1
		elseif self.das_down then
			change = 1
		end
		self:changeOption(change)
		self.das = self.das - config.menu_arr
	end
end

function RecordsScene:render()
	love.graphics.setColor(1, 1, 1, 1)
	drawBackground(0)

	love.graphics.setFont(font_8x11)
	love.graphics.print("RECORDS", 80, 43)

	local b = cursorHighlight(20, 40, 50, 30)
	love.graphics.setColor(1, 1, b, 1)
	love.graphics.printf(chars.big_left, 20, 40, 50, "center")
	love.graphics.setColor(1, 1, 1, 1)

	love.graphics.setFont(font_3x5_2)
	love.graphics.print("Take a gander at your card and records!", 80, 90)

	love.graphics.setColor(1, 1, 1, 0.5)
	love.graphics.rectangle("fill", 75, 248 + 40 * self.menu_state, font_3x5_3:getWidth(menu_screens[self.menu_state].title) + 16, 33)

	PlayerCardScene.renderCard(80, 130)

	love.graphics.setFont(font_3x5_3)
	love.graphics.setColor(1, 1, 1, 1)
	for i, screen in pairs(menu_screens) do
		local b = cursorHighlight(80,240 + 40 * i,200,40)
		love.graphics.setColor(1,1,b,1)
		love.graphics.printf(screen.title, 80, 250 + 40 * i, 200, "left")
	end
end

function RecordsScene:changeOption(rel)
	local len = #menu_screens
	self.menu_state = (self.menu_state + len + rel - 1) % len + 1
	playSE("cursor")
end

function RecordsScene:onInputPress(e)
	if self.safety_frames > 0 then return end
	if e.type == "mouse" then
		if cursorHoverArea(20, 40, 50, 30) then
			playSE("menu_cancel")
			saveConfig()
			scene = TitleScene()
		end
		if cursorHoverArea(50, 280, 200, #menu_screens * 40) then
			self.menu_state = math.floor((e.y - 240) / 40)
			playSE("main_decide")
			scene = menu_screens[self.menu_state]()
		end
	end
	if e.input == "menu_decide" then
		playSE("main_decide")
		scene = menu_screens[self.menu_state]()
	elseif e.input == "menu_up" then
		self:changeOption(-1)
		self.das_up = true
	elseif e.input == "menu_down" then
		self:changeOption(1)
		self.das_down = true
	elseif e.input == "menu_back" or e.scancode == "backspace" or e.scancode == "delete" then
		playSE("menu_cancel")
		scene = TitleScene()
	end
end

function RecordsScene:onInputRelease(e)
	if e.input == "menu_up" then
		self.das_up = false
	elseif e.input == "menu_down" then
		self.das_down = false
	end
end

return RecordsScene